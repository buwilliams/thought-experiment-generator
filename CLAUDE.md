...
cargo run -- ask "question"                                               # ask the system; answered by best lens
...
- `ask.rs` — `ask()` loads mind + candidates, finds the top conjecture by composite score across both layers, and generates a direct answer using the `ask.md` prompt. Prints the answer followed by the active lens and its composite score.
