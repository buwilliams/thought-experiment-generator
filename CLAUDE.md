...
- `state.rs` — all filesystem I/O. Reads/writes `.md` + `.json` sidecar pairs for conjectures, problems, and generated outputs. `ensure_initialized()` copies seed to state on first run. Key functions: `load_conjectures`/`save_conjecture`/`delete_conjecture` for mind/candidates; `save_generated`/`load_run_generated`/`generated_exists` for ephemeral run output.
