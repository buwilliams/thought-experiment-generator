...
The Thought Experiment Generator is a system for making informed guesses. It takes a topic context — anything from a single phrase to a multi-page document — extracts background knowledge from it, and collides that knowledge with sentences generated from random words. The randomness is a forcing function to reach territory that pure topic knowledge would never find on its own. The richer the topic context, the more precisely the background knowledge is targeted, and the more specific the collisions.
...
**`background.txt`** — A pool of sentences drawn from the topic context, biased toward anomalies, unresolved tensions, edge cases, and open questions rather than established textbook facts. Generated by a single LLM call. The topic context can be a short phrase, a paragraph, or a full document supplied via `--topic-file` or stdin.

**`words.txt`** — Lines of random words sampled from `data/words_alpha.txt`. Each line has `--words` words (default: 5). The number of lines equals `--pool-size` (default: 50). The word list is ~17k common English words (no proper nouns, no stopwords), sourced from the intersection of the ENABLE dictionary and a frequency corpus.

**`generated.txt`** — Each line of `words.txt` converted into a grammatically correct, meaningful sentence. The sentence connects the words into a coherent claim or observation, but the combination of concepts across words is intentionally unexpected. Generated by a single LLM call processing all lines at once.
...
**User:** Turn each line into a single grammatically correct, meaningful sentence that uses all the words on that line. The sentence should make sense and connect the words into a coherent claim or observation — but the combination of concepts can be unexpected or surprising. Lines: {word_lines}
