...
5. [LLM call budget] - ~357 calls per run at default settings (5 problems × ~10 lenses × 7 calls per pair). If the evaluation passes are not all adding signal, some calls are wasted. Worth monitoring once the new prompts have been tested — if some passes prove redundant, the budget can be cut.
